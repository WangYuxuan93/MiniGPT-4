...
def parse_args():
    ...
    parser.add_argument("--input-path", required=True, help="path to input json file.")
    parser.add_argument("--img-dir", required=True, help="directory to image path.")
...
output = []
img_dir = args.img-dir
cnt = 0
with open(args.input-path, "r", encoding="utf-8") as f:
    data = json.load(f)
    for item in data:
        cnt += 1
        ...
        query = item["question"]
        pred = ask_llm(img_path, query)
        item["pred"] = pred
        output.append(item)
with open(args.output-path, "w") as f:
    json.dump(output, f, ident=2)
